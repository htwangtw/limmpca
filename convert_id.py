...
for _, subject in id_ref.iterrows():
    ...
    dates_list = [re.search(r"\d{4}_[a-zA-Z]{3}_\d{2}_\d{4}", f.name)[0] 
                  for f in files]
    dates_list = sorted((datetime.strptime(d, "%Y_%b_%d_%H%M") for d in dates_list))

    # add session number and R number in the file
    for i, d in enumerate(dates_list):
        f = list(path_data.glob(f"{ref_idno}_*_{d.strftime('%Y_%b_%d_%H%M')}*.csv"))
        assert len(f) == 1
...
for cur in subject_logs:
    ...
    for i in real_probe_idx:
        ...
        time_probe = cur.loc[i, "fixStart"] + cur.loc[i, "fixT"]
        stim = "NT"
        stim_i = i
        while stim == "NT" and stim_i != 0:
            ...
            time_stim = cur.loc[stim_i, "fixStart"] + cur.loc[stim_i, "fixT"]
            if stim == "MWQ":
                stim_i += 1
                time_stim = cur.loc[stim_i, "fixStart"]
            corr = cur.loc[stim_i, "respCORR"]
        if stim == "NT":
            stim = "task_start"
            print(cur["IDNO"][0])
        time_probe -= time_stim
        probe["corr"] = corr
        probe["interval"] = time_probe
        probe["prev_task"] = stim
        probes.append(probe)
    probes = pd.concat(probes).reset_index(drop=True)
    probes["RNO"] = cur["RNO"][0]
